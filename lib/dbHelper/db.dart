...
class ObjectBox {
  ...
  // METHOD TO INSERT SPECIFIC CHAPTER TO  DATABASE.
  static Future<void> saveStatus(List<Status> status) async {
    objectBox.statusBox.putMany(status);
  }

  // METHOD TO INSERT SPECIFIC CHAPTER TO  DATABASE.
  static Future<int> statusCount() async {
    final query = objectBox.statusBox.query(Status_.shown.equals("0")).build();
    return query.count();
  }

  // Method to get all registered courses from database.
  static Future<List<Status>> getAllStatus() async {
    return objectBox.statusBox.getAll();
  }

  //METHOD TO UPDATE QUESTION TABLE OF USER PROGRESS
  static Future<int> viewedStatus(id) async {
    final query = objectBox.statusBox.query(Status_.url.equals(id)).build();
    ...
    Status que = Status(
        ...
        shown: "1",
        level: result.level);
    return objectBox.statusBox.put(que);
  }

  //METHOD TO UPDATE QUESTION TABLE OF USER PROGRESS
  static Future<int> deleteStatusAfter24hrs() async {
    final query = objectBox.statusBox
        .query(Status_.time.lessThan(DateTime.now()
            .subtract(Duration(hours: 24))
            .millisecondsSinceEpoch))
        .build();
    final result = query.findIds();

    return objectBox.statusBox.removeMany(result);
  }
  ...
  static Future<List<int>> saveQuestion(List<Question> questions) async {
    return objectBox.questionBox.putMany(questions);
  }
